...
# 
    ...
    #We want to test on sentences we haven't trained on, to see how well the model generalises to previously unseen sentences
    ...
    for sentence, sentiment in sentencesTrain.items():
        ...
        for word in wordList: #calculate over unigrams
            ...
            if sentiment=="positive" :
                ...
                #keep count of each word in negative context
    ...
    for sentence, sentiment in sentencesTest.items():
        ...
        if sentiment=="positive":
            ...
            if score>=threshold:
                ...
                correct+=0
                totalnegpred+=1
                #print ("ERROR (pos classed as neg %f):" %score + sentence)
        else:
            totalneg+=1
            if score<threshold:
                correct+=1
                ...
                correct+=0
                totalpospred+=1
                #print ("ERROR (neg classed as pos %f):" %score + sentence)
...
# This is the new rule based approach

def testDictionary2(sentencesTest, dataName, sentimentDictionary, threshold):
    total=0
    correct=0
    totalpos=0
    totalneg=0
    ...
    # capturing all the intensifiers
    intensifiers=[]
    for word in posWordList:
        if re.findall(r"[\w']+ly$",word):
            intensifiers.append(word)
    # capturing all the diminishers
    diminishers=[]
    for word in negWordList:
        if re.findall(r"[\w']+ly$",word):
            diminishers.append(word)
    
    
    
    sentimentDictionary2={}
    for sentword in sentimentDictionary:
        capitalword=sentword.capitalize()
        sentimentDictionary2[capitalword]=sentimentDictionary[sentword]
    
    sentimentDictionary2= sentimentDictionary | sentimentDictionary2
    
    for sentence, sentiment in sentencesTest.items():
        Words = re.findall(r"[\w']+", sentence)
        score=0
        for word in Words:
            if word in sentimentDictionary2:
                #print(sentimentDictionary2[word],word,"step1")
                score+=sentimentDictionary2[word]
               
        # negation rule    
        for word in Words:
            if word in sentimentDictionary2:
                if sentimentDictionary2[word]<0 & score>0:
                    #print(sentimentDictionary2[word],word,"neg")
                    score=score*(-1)
        ...
        for word in Words:
            if word in sentimentDictionary2:
                if word.isupper():
                    if sentimentDictionary2[word]>0:
                        score+=1
                        #print(sentimentDictionary2[word],word,"cap+ve")
                    else:
                        score=score-1
                        #print(sentimentDictionary2[word],word,"cap-ve")
        ...
        for word in Words:
            if word in intensifiers:
                ...
                try:
                    if Words[loc+1] in sentimentDictionary2:
                        if sentimentDictionary2[Words[loc+1]]>0:
                            score+=1
                            #print(1,word,"int+ve")
                        elif sentimentDictionary2[Words[loc+1]]<0:
                            score=score-1
                            #print(-1,word,"int-ve")
                except:
                    False
        ...
        for word in Words:
            if word in diminishers:
                ...
                try:
                    if Words[loc+1] in sentimentDictionary2:
                        if sentimentDictionary2[Words[loc+1]]==1:
                            score=score-1
                            #print(-1,word,"dim+ve")
                        elif sentimentDictionary2[Words[loc+1]]==-1:
                            score+=1
                            #print(1,word,"dim-ve")
                except:
                    False
...
# testDictionary2(sentencesTrain,  "Films (Train Data, Rule-Based)\t", sentimentDictionary, +4)
# testDictionary2(sentencesTest,  "Films  (Test Data, Rule-Based)\t",  sentimentDictionary, +1)
# testDictionary2(sentencesNokia, "Nokia   (All Data, Rule-Based)\t",  sentimentDictionary, +1)
